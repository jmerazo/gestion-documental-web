...
var source = {
    ...
    templates: {
        ...
        views: ['app/views/**/*.html'] // Cambia la ruta aquí a app/views
...
gulp.task('vendor:base', function () {
    ...
    return gulp.src(vendor.base.source)
        ...
        .pipe($.if(true, $.cssnano(cssnanoOpts)))
...
gulp.task('vendor:app', function () {
    ...
    return gulp.src(vendor.app.source, {
        ...
        .pipe(jsFilter)
        .pipe($.if(true, $.uglify(vendorUglifyOpts)))
        .pipe(jsFilter.restore)
        .pipe(cssFilter)
...
gulp.task('browsersync', function () {
    log('Starting BrowserSync with Apache proxy..');

    browserSync.init({
        proxy: "http://gesdoc.local", // Esto apunta a tu vhost en Apache
        port: 3010, // Puedes cambiar este puerto si es necesario
        notify: false
    });

    gulp.watch(source.scripts, ['scripts:app']).on('change', reload);
    gulp.watch(source.styles.watch, ['styles:app', 'styles:app:rtl']).on('change', reload);
    gulp.watch(source.styles.themes, ['styles:themes']).on('change', reload);
    gulp.watch(source.templates.views, ['templates:views']).on('change', reload);
    gulp.watch(source.templates.index, ['templates:index']).on('change', reload);
});
...
gulp.task('templates:views', function () {
    log('Copying HTML views..');

    return gulp.src(source.templates.views)
        .pipe(gulp.dest(build.templates.views)) // Copiar las vistas HTML a la carpeta de destino
        .pipe(reload({ stream: true }));  // Recargar el navegador
});
